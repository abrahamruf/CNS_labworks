...
public class hill
{
    ...
    {
        key_mtx = new int[len][len];
        int c = 0;
        for (int i = 0; i < len; i++)
        {
            for (int j = 0; j < len; j++)
            {
                key_mtx[i][j] = ((int) key.charAt(c)) - 97;
                c++;
            }
        }
    }
    ...
    {
        line_mtx = new int[line.length()];
        for (int i = 0; i < line.length(); i++)
        {
            line_mtx[i] = ((int) line.charAt(i)) - 97;
        }
    }
     public void linemultiplykey(int len)
    {
        rslt_mtx = new int[len];
        for (int i = 0; i < len; i++)
        {
            for (int j = 0; j < len; j++)
            {
                rslt_mtx[i] += key_mtx[i][j] * line_mtx[j];
            }
            rslt_mtx[i] %= 26;
        }
    }
     public void rslt(int len)
    {
        String rslt = "";
        for (int i = 0; i < len; i++)
        {
            rslt += (char) (rslt_mtx[i] + 97);
        }
