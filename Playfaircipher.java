...
class PFEncryption{
private char [][] alph= new char[5][5];
private char[] uniq_char= new char[26];
...
void makeArray(String keyword){
    ...
    for (int i=0; i<keyword.length(); i++){
        ...
        uniq_length=0;
        if (keyword.charAt(i)!= ' '){
            for (int k=0; k<uniq_char.length; k++){
                if (Character.toString(uniq_char[k])==null){
                    break;
                }
                uniq_length++;
            }
            for (int j=0; j<uniq_char.length; j++){
                if (keyword.charAt(i)==uniq_char[j]){
                    present=true;
                }
            }
            if (!present){
                uniq_char[value]=keyword.charAt(i);
                value++;
            }
        }
    ...
    for (int i=0; i<ch.length(); i++){
        uniq_char[value]=ch.charAt(i);
        value++;
    }
    value=0;
    for (int i=0; i<5; i++){
        for (int j=0; j<5; j++){
            alph[i][j]=uniq_char[value];
            value++;
...
void encrypt(int row1, int col1, int row2, int col2){
    if (row1==row2){
        ...
        if (col2>4)
            col2=0;
 enc+=(Character.toString(alph[row1][col1])+Character.toString(alph[row1][col2]));
    }else if(col1==col2){
        row1=row1+1;
        row2=row2+1;
        if (row1>4)
        ...
        if (row2>4)
            row2=0;
 enc+=(Character.toString(alph[row1][col1])+Character.toString(alph[row2][col1]));
    }else{
        enc+=(Character.toString(alph[row1][col2])+Character.toString(alph[row2][col1]));
    }
}
void decrypt(int row1, int col1, int row2, int col2){
    if (row1==row2){
        ...
        if (col2<0)
            col2=4;
        dec+=(Character.toString(alph[row1][col1])+Character.toString(alph[row1][col2]));
    }else if(col1==col2){
        row1=row1-1;
        row2=row2-1;
        if (row1<0)
        ...
        if (row2<0)
            row2=4;
        dec+=(Character.toString(alph[row1][col1])+Character.toString(alph[row2][col1]));
    }else{
        dec+=(Character.toString(alph[row1][col2])+Character.toString(alph[row2][col1]));
    }
}
String getEncrypted(){
    return enc;
